#!/usr/bin/env bash
set -Eeuo pipefail
IFS=$'\n\t'
umask 077

readonly SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
readonly COMPOSE_FILE="$SCRIPT_DIR/compose.yaml"
readonly INSTANCES_DIR="$SCRIPT_DIR/instances"
readonly SKILLS_DIR="$SCRIPT_DIR/skills"
readonly SHARED_TAILSCALE_AUTHKEY_FILE="$SCRIPT_DIR/.tailscale-authkey"

info() { printf '\033[1;34m==>\033[0m %s\n' "$*"; }
warn() { printf '\033[1;33mwarning:\033[0m %s\n' "$*" >&2; }
die() { printf '\033[1;31merror:\033[0m %s\n' "$*" >&2; exit "${ERROR_EXIT_CODE:-1}"; }

usage() {
  cat <<'USAGE'
Hermes Spawning — isolated Hermes Agent + Tailscale environments

Usage:
  ./hermes-spawn.sh spawn                 Create and launch an instance
  ./hermes-spawn.sh list                  List instances
  ./hermes-spawn.sh start [instance]      Start an existing instance
  ./hermes-spawn.sh stop [instance]       Stop it (persistent data remains)
  ./hermes-spawn.sh restart [instance]    Restart it
  ./hermes-spawn.sh status [instance]     Show containers and Tailnet status
  ./hermes-spawn.sh logs [instance]       Follow both containers' logs
  ./hermes-spawn.sh setup [instance]      Run Hermes' setup wizard
  ./hermes-spawn.sh config [instance]     Alias for setup
  ./hermes-spawn.sh chat [instance]       Open the selected Hermes profile
  ./hermes-spawn.sh shell [instance]      Open a shell inside Hermes
  ./hermes-spawn.sh doctor [instance]     Run Hermes diagnostics
  ./hermes-spawn.sh serve [instance]      Configure private Tailnet HTTPS
  ./hermes-spawn.sh reauth [instance]     Replace an expired Tailscale login
  ./hermes-spawn.sh credentials [instance] Show URLs, API key, and web login
  ./hermes-spawn.sh update [instance]     Pull images and recreate containers
  ./hermes-spawn.sh remove [instance]     Delete an instance (archives first)
  ./hermes-spawn.sh apply-stack [instance]  Apply the fleet model-stack defaults
  ./hermes-spawn.sh stack-status [instance] Audit the defaults; exit 1 on drift
  ./hermes-spawn.sh audit-isolation [instance] Audit cross-node bleed; exit 1 on findings
  ./hermes-spawn.sh install-skills [instance] Install/refresh the fleet skills (skills/)
  ./hermes-spawn.sh webui-ensure [instance]  Deploy+launch the instance webui chat UI
  ./hermes-spawn.sh webui-status [instance]  Show webui status
  ./hermes-spawn.sh supervisor install|enable|disable|status
                                     Fleet webui/routes autostart (systemd)
  ./hermes-spawn.sh info [instance]       Show the collected host snapshot
  ./hermes-spawn.sh host-preflight        Check host requirements only
  ./hermes-spawn.sh preflight instance [--json]  Read-only fleet readiness check
  ./hermes-spawn.sh fleet-doctor instance|all [--json] [--policy-file PATH]
                                     Read-only disk audit; exit 1 on proven blockers
  ./hermes-spawn.sh formation-status instance|all
                                     Read-only formation status; never claims qualification
  ./hermes-spawn.sh add-persona instance name   Create and verify a clean persona

Stack defaults live in stack-defaults.yaml. Re-run apply-stack after adding
personas manually (add-persona applies them automatically). Run ./deploy.sh first to bring up the
host model stack (CCS + CLIProxy + OAuth + Chutes) that endpoints point at.

An instance name is its Tailscale hostname. If omitted, the script selects the
only instance or prompts when several exist.

remove asks you to type the instance name, archives the directory to
.cache/<name>-removed-<stamp>.tar.gz, then deletes containers and data.
Flags: -y skips the prompt, --keep-data removes containers but keeps files.
Deleting the local files does not delete the Tailscale machine: remove it at
https://login.tailscale.com/admin/machines or the name stays taken and the
next spawn of that name becomes <name>-1.

Use 'all' (or -a) in place of the name to run a command on every instance,
one after another, for example:
  ./hermes-spawn.sh restart all
  ./hermes-spawn.sh apply-stack all -y
  ./hermes-spawn.sh status all
Each instance keeps its own lock and validation. A failure on one instance
does not stop the rest; the exit code is 1 if any instance failed.
Interactive commands (chat, shell, logs, setup, config, reauth) refuse 'all'.

Fleet skills live in skills/ (each is a directory with a SKILL.md). Spawn
installs them into hermes-data/fleet-skills/ automatically; install-skills pushes
them to an existing instance and is safe to re-run. See the README.
USAGE
}

require_command() {
  command -v "$1" >/dev/null 2>&1 || die "Required command not found: $1"
}

validate_hostname() {
  [[ "$1" =~ ^[a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?$ ]]
}

validate_profile() {
  [[ "$1" =~ ^[A-Za-z0-9][A-Za-z0-9_-]{0,47}$ ]] || return 1
  case "$1" in
    hermes|test|tmp|root|sudo) return 1 ;;
  esac
}

validate_cpu() {
  [[ "$1" =~ ^[0-9]+([.][0-9]+)?$ ]] && awk -v n="$1" 'BEGIN { exit !(n > 0) }'
}

validate_memory() {
  [[ "$1" =~ ^[1-9][0-9]*([kmgtKMGT]|[kmgtKMGT][bB])$ ]]
}

prompt_default() {
  local prompt="$1" default="$2" value
  read -r -p "$prompt [$default]: " value
  printf '%s' "${value:-$default}"
}

prompt_secret() {
  local prompt="$1" value
  read -r -s -p "$prompt" value
  printf '\n' >&2
  printf '%s' "$value"
}

tailscale_authkey() {
  local prompt="$1" value mode
  if [[ -s "$SHARED_TAILSCALE_AUTHKEY_FILE" ]]; then
    mode="$(stat -c '%a' "$SHARED_TAILSCALE_AUTHKEY_FILE" 2>/dev/null || true)"
    [[ "$mode" == 600 ]] || die "$SHARED_TAILSCALE_AUTHKEY_FILE must have mode 0600."
    value="$(<"$SHARED_TAILSCALE_AUTHKEY_FILE")"
    [[ "$value" != *$'\n'* && "$value" == tskey-* ]] || die "Invalid reusable key in $SHARED_TAILSCALE_AUTHKEY_FILE"
    info "Using the reusable Tailscale auth key from $SHARED_TAILSCALE_AUTHKEY_FILE" >&2
  else
    value="$(prompt_secret "$prompt")"
  fi
  [[ "$value" == tskey-* ]] || die "A Tailscale auth key (tskey-...) is required."
  printf '%s' "$value"
}

confirm() {
  local prompt="$1" default="${2:-yes}" answer suffix
  if [[ "$default" == yes ]]; then suffix='[Y/n]'; else suffix='[y/N]'; fi
  read -r -p "$prompt $suffix " answer
  answer="${answer:-$default}"
  [[ "$answer" =~ ^[Yy]([Ee][Ss])?$ ]]
}

host_cpu_count() {
  getconf _NPROCESSORS_ONLN 2>/dev/null || nproc 2>/dev/null || printf '4\n'
}

host_memory_mib() {
  awk '/^MemTotal:/ { print int($2 / 1024); exit }' /proc/meminfo 2>/dev/null || printf '8192\n'
}

default_cpu_limit() {
  local total="$1"
  if (( total >= 8 )); then printf '%s.0\n' "$((total - 1))"
  elif (( total >= 2 )); then printf '%s.0\n' "$total"
  else printf '1.0\n'; fi
}

default_memory_limit() {
  local total_mib="$1" gib
  gib=$((total_mib * 80 / 100 / 1024))
  (( gib < 2 )) && gib=2
  printf '%sg\n' "$gib"
}

host_preflight() {
  require_command docker
  require_command awk
  require_command flock
  require_command openssl
  [[ -f "$COMPOSE_FILE" ]] || die "Missing $COMPOSE_FILE"
  docker info >/dev/null 2>&1 || die "Docker daemon is not reachable by this user."
  docker compose version >/dev/null 2>&1 || die "Docker Compose v2 is required."
  [[ -c /dev/net/tun ]] || die "/dev/net/tun is missing; load the tun module before spawning."
  info "Docker, Compose, and /dev/net/tun are ready."
}

instance_dirs() {
  local path
  shopt -s nullglob
  for path in "$INSTANCES_DIR"/*/control.env; do dirname -- "$path"; done
  shopt -u nullglob
}

list_instances() {
  local dir any=0 state profile
  printf '%-28s %-20s %-12s\n' HOSTNAME PROFILE STATE
  while IFS= read -r dir; do
    [[ -n "$dir" ]] || continue
    any=1
    COMPOSE_PROJECT_NAME="$(read_control_value COMPOSE_PROJECT_NAME "$dir/control.env")"
    state="stopped"
    if docker ps --format '{{.Label "com.docker.compose.project"}}' 2>/dev/null | grep -Fqx "$COMPOSE_PROJECT_NAME"; then
      state="running"
    fi
    profile="$(read_control_value HERMES_PROFILE "$dir/control.env")"
    profile="${profile:-unknown}"
    printf '%-28s %-20s %-12s\n' "$(basename "$dir")" "$profile" "$state"
  done < <(instance_dirs)
  (( any == 1 )) || printf '%s\n' '(no instances yet)'
}

read_control_value() {
  local key="$1" file="$2"
  awk -v wanted="$key" '
    index($0, wanted "=") == 1 {
      print substr($0, length(wanted) + 2)
      exit
    }
  ' "$file"
}

resolve_instance() {
  local requested="${1:-}" dir count=0 only="" choice
  if [[ -n "$requested" ]]; then
    validate_hostname "$requested" || die "Invalid instance name: $requested"
    dir="$INSTANCES_DIR/$requested"
    [[ -f "$dir/control.env" ]] || die "Unknown instance: $requested"
    printf '%s\n' "$dir"
    return
  fi
  while IFS= read -r dir; do
    [[ -n "$dir" ]] || continue
    only="$dir"
    count=$((count + 1))
  done < <(instance_dirs)
  (( count > 0 )) || die "No instances exist. Run: ./hermes-spawn.sh spawn"
  if (( count == 1 )); then printf '%s\n' "$only"; return; fi
  list_instances >&2
  read -r -p 'Instance hostname: ' choice
  validate_hostname "$choice" || die "Invalid instance name: $choice"
  dir="$INSTANCES_DIR/$choice"
  [[ -f "$dir/control.env" ]] || die "Unknown instance: $choice"
  printf '%s\n' "$dir"
}

load_instance() {
  local dir="$1"
  local control="$dir/control.env" key value
  local keys=(
    COMPOSE_PROJECT_NAME TAILSCALE_HOSTNAME TAILSCALE_STATE_DIR
    TAILSCALE_AUTHKEY_FILE TS_AUTHKEY_SPEC HERMES_DATA_DIR HERMES_PROFILE
    HERMES_DISPLAY_NAME
    HERMES_UID HERMES_GID HERMES_CPUS HERMES_MEMORY
    HERMES_MEMORY_RESERVATION HERMES_SHM_SIZE HERMES_PIDS_LIMIT
    HERMES_IMAGE TAILSCALE_IMAGE
    MODEL_STACK_BASE_URL MODEL_STACK_API_KEY
  )
  for key in "${keys[@]}"; do
    value="$(read_control_value "$key" "$control")"
    printf -v "$key" '%s' "$value"
  done
  validate_hostname "$TAILSCALE_HOSTNAME" || die "Unsafe hostname in $control"
  validate_profile "$HERMES_PROFILE" || die "Unsafe profile in $control"
  [[ -z "$HERMES_DISPLAY_NAME" ]] || validate_profile "$HERMES_DISPLAY_NAME" \
    || die "Unsafe display name in $control"
  [[ "$COMPOSE_PROJECT_NAME" == "hermes-$TAILSCALE_HOSTNAME" ]] || die "Unsafe project name in $control"
  [[ "$TAILSCALE_STATE_DIR" == "$dir/tailscale-state" ]] || die "Unexpected Tailscale state path in $control"
  [[ "$TAILSCALE_AUTHKEY_FILE" == "$dir/tailscale-authkey" ]] || die "Unexpected auth-key path in $control"
  [[ "$TS_AUTHKEY_SPEC" == file:/run/secrets/tailscale-authkey ]] || die "Unsafe auth-key setting in $control"
  [[ "$HERMES_DATA_DIR" == "$dir/hermes-data" ]] || die "Unexpected Hermes data path in $control"
  [[ "$HERMES_UID" =~ ^[1-9][0-9]*$ && "$HERMES_GID" =~ ^[1-9][0-9]*$ ]] || die "Unsafe UID/GID in $control"
  validate_cpu "$HERMES_CPUS" || die "Unsafe CPU limit in $control"
  validate_memory "$HERMES_MEMORY" || die "Unsafe memory limit in $control"
  validate_memory "$HERMES_MEMORY_RESERVATION" || die "Unsafe memory reservation in $control"
  validate_memory "$HERMES_SHM_SIZE" || die "Unsafe shared-memory size in $control"
  [[ "$HERMES_PIDS_LIMIT" =~ ^[1-9][0-9]*$ ]] || die "Unsafe PID limit in $control"
  [[ -n "$HERMES_IMAGE" && -n "$TAILSCALE_IMAGE" ]] || die "Missing image setting in $control"
  [[ -z "$MODEL_STACK_BASE_URL" || "$MODEL_STACK_BASE_URL" =~ ^https?://[A-Za-z0-9.:-]+(/v1)?$ ]] || die "Unsafe MODEL_STACK_BASE_URL in $control"
  [[ -z "$MODEL_STACK_API_KEY" || ( "$MODEL_STACK_API_KEY" =~ ^[A-Za-z0-9._~+:/=-]{1,220}$ ) ]] || die "Unsafe MODEL_STACK_API_KEY in $control"
  export COMPOSE_PROJECT_NAME TAILSCALE_HOSTNAME TAILSCALE_STATE_DIR
  export TAILSCALE_AUTHKEY_FILE TS_AUTHKEY_SPEC HERMES_DATA_DIR HERMES_PROFILE
  export HERMES_DISPLAY_NAME
  export HERMES_UID HERMES_GID HERMES_CPUS HERMES_MEMORY
  export HERMES_MEMORY_RESERVATION HERMES_SHM_SIZE HERMES_PIDS_LIMIT
  export HERMES_IMAGE TAILSCALE_IMAGE
  export MODEL_STACK_BASE_URL MODEL_STACK_API_KEY
}

compose() {
  docker compose --env-file "$INSTANCE_DIR/control.env" -f "$COMPOSE_FILE" "$@"
}

# One-shot CLI commands must bypass the image's service supervisor. Recent
# Hermes images otherwise also start the dashboard, which can keep
# `docker compose run` alive after the requested command has completed.
hermes_run() {
  compose run --rm --no-deps --entrypoint hermes hermes "$@"
}

collect_host_info() {
  {
    printf 'collected_at=%s\n' "$(date --iso-8601=seconds)"
    printf 'host=%s\n' "$(hostname -f 2>/dev/null || hostname)"
    printf 'kernel=%s\n' "$(uname -srmo)"
    printf 'architecture=%s\n' "$(uname -m)"
    printf 'cpu_count=%s\n' "$(host_cpu_count)"
    printf 'memory_mib=%s\n' "$(host_memory_mib)"
    printf 'workspace_filesystem='; df -hP "$SCRIPT_DIR" | awk 'NR==2 {print $2 " total, " $4 " available, " $6 " mount"}'
    printf 'docker='; docker version --format 'client={{.Client.Version}} server={{.Server.Version}}' 2>/dev/null || printf 'unavailable\n'
    printf 'compose='; docker compose version --short 2>/dev/null || printf 'unavailable\n'
    printf 'docker_security='; docker info --format '{{join .SecurityOptions ","}}' 2>/dev/null || printf 'unknown\n'
    if command -v nvidia-smi >/dev/null 2>&1; then
      printf 'gpu='; nvidia-smi --query-gpu=name,memory.total --format=csv,noheader 2>/dev/null | paste -sd ';' - || true
    else
      printf 'gpu=not-detected\n'
    fi
  } > "$1"
  chmod 600 "$1"
}

HERMES_DISPLAY_NAME="${HERMES_DISPLAY_NAME:-}"

write_control_file() {
  local target="$1"
  {
    printf 'COMPOSE_PROJECT_NAME=%s\n' "$COMPOSE_PROJECT_NAME"
    printf 'TAILSCALE_HOSTNAME=%s\n' "$TAILSCALE_HOSTNAME"
    printf 'TAILSCALE_STATE_DIR=%s\n' "$TAILSCALE_STATE_DIR"
    printf 'TAILSCALE_AUTHKEY_FILE=%s\n' "$TAILSCALE_AUTHKEY_FILE"
    printf 'TS_AUTHKEY_SPEC=%s\n' "$TS_AUTHKEY_SPEC"
    printf 'HERMES_DATA_DIR=%s\n' "$HERMES_DATA_DIR"
    printf 'HERMES_PROFILE=%s\n' "$HERMES_PROFILE"
    printf 'HERMES_DISPLAY_NAME=%s\n' "${HERMES_DISPLAY_NAME:-}"
    printf 'HERMES_UID=%s\n' "$HERMES_UID"
    printf 'HERMES_GID=%s\n' "$HERMES_GID"
    printf 'HERMES_CPUS=%s\n' "$HERMES_CPUS"
    printf 'HERMES_MEMORY=%s\n' "$HERMES_MEMORY"
    printf 'HERMES_MEMORY_RESERVATION=%s\n' "$HERMES_MEMORY_RESERVATION"
    printf 'HERMES_SHM_SIZE=%s\n' "$HERMES_SHM_SIZE"
    printf 'HERMES_PIDS_LIMIT=%s\n' "$HERMES_PIDS_LIMIT"
    printf 'HERMES_IMAGE=%s\n' "$HERMES_IMAGE"
    printf 'TAILSCALE_IMAGE=%s\n' "$TAILSCALE_IMAGE"
    printf 'MODEL_STACK_BASE_URL=%s\n' "${MODEL_STACK_BASE_URL:-}"
    printf 'MODEL_STACK_API_KEY=%s\n' "${MODEL_STACK_API_KEY:-}"
  } > "$target"
  chmod 600 "$target"
}

wait_for_tailscale() {
  local attempt state
  info "Waiting for $TAILSCALE_HOSTNAME to join the tailnet..."
  for attempt in {1..60}; do
    state="$(compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock status --json 2>/dev/null || true)"
    if grep -Eq '"BackendState"[[:space:]]*:[[:space:]]*"Running"' <<< "$state"; then
      info "Tailscale is connected."
      : > "$TAILSCALE_AUTHKEY_FILE"
      chmod 600 "$TAILSCALE_AUTHKEY_FILE"
      return 0
    fi
    sleep 2
  done
  compose logs --tail=50 tailscale >&2 || true
  die "Tailscale did not become ready within two minutes. Fix authentication, then run start."
}

profile_env_path() {
  if [[ "$HERMES_PROFILE" == default ]]; then
    printf '%s/.env\n' "$HERMES_DATA_DIR"
  else
    printf '%s/profiles/%s/.env\n' "$HERMES_DATA_DIR" "$HERMES_PROFILE"
  fi
}

# A named profile without its own profile.yaml inherits the instance root's
# ui_meta (the default profile's), including any Desktop room registry. Write a
# minimal local profile.yaml so the named profile shadows the parent instead.
# Report cross-node bleed: named profiles inheriting the root's ui_meta, and
# bot_relay rosters holding agents from other nodes. Exit 1 on any finding.
audit_isolation_command() {
  local findings=0 root_pf="${HERMES_DATA_DIR}/profile.yaml" pdir name pf roster
  if [[ -f "$root_pf" ]] && grep -q 'hermes-bots-groups' "$root_pf" 2>/dev/null; then
    for pdir in "${HERMES_DATA_DIR}"/profiles/*/; do
      [[ -d "$pdir" ]] || continue
      name="$(basename -- "$pdir")"
      [[ "$name" == .deleted ]] && continue
      pf="${pdir}profile.yaml"
      if [[ ! -f "$pf" ]]; then
        warn "INHERITS root ui_meta: profile '${name}' has no profile.yaml"
        findings=$((findings + 1))
      fi
    done
  fi
  # The roster file is only a symptom; what matters is whether agents from
  # OTHER instances reach this instance's system prompt. Ask Hermes directly.
  local relay_dir="${HERMES_DATA_DIR}/bot_relay" remote_count perms
  roster="${relay_dir}/roster.json"
  remote_count="$(compose exec -T hermes python3 -c \
    'import sys; sys.path.insert(0, "/opt/hermes")
from pathlib import Path
from tools.bot_relay import read_remote_roster
print(len(read_remote_roster(Path("/opt/data"))))' 2>/dev/null | tr -d "[:space:]")" || { warn "Cannot inspect native relay roster"; return 2; }
  [[ "$remote_count" =~ ^[0-9]+$ ]] || { warn "Invalid native relay roster result"; return 2; }
  if [[ "$remote_count" =~ ^[0-9]+$ ]] && (( remote_count > 0 )); then
    warn "CROSS-INSTANCE: ${remote_count} agent(s) from other instances are in this prompt"
    findings=$((findings + 1))
  fi
  if [[ "${HERMES_ALLOW_CROSS_INSTANCE_RELAY:-no}" != yes ]]; then
    # A MISSING relay dir is not safe: Hermes creates it unlocked on first use,
    # so the next desktop connection could seed a foreign roster. Treat absent
    # and writable the same way -- both need lock_bot_relay.
    if [[ ! -d "$relay_dir" ]]; then
      warn "Relay dir is missing (unlocked on first use); run: ./hermes-spawn.sh start ${TAILSCALE_HOSTNAME}"
      findings=$((findings + 1))
    else
      perms="$(stat -c '%a' "$relay_dir" 2>/dev/null || echo '')"
      if [[ "$perms" != 555 ]]; then
        warn "Relay dir is writable (mode ${perms:-?}); run: ./hermes-spawn.sh start ${TAILSCALE_HOSTNAME}"
        findings=$((findings + 1))
      fi
    fi
  fi
  if (( findings == 0 )); then
    info "Isolation audit clean for ${TAILSCALE_HOSTNAME}."
  fi
  return $(( findings > 0 ))
}

# Bot Mode is per-instance: profiles inside ONE instance are teammates and may
# message each other. A desktop client connected to several instances also
# pushes each instance a roster of the OTHER instances' agents, which lands in
# their system prompt. Instances are separate containers and are not one crew,
# so the relay drop directory is kept root-owned and read-only: the local
# roster stays empty and cross-instance pushes fail closed. Bot Mode itself is
# untouched. Set HERMES_ALLOW_CROSS_INSTANCE_RELAY=yes to opt in.
lock_bot_relay() {
  local relay_dir="${HERMES_DATA_DIR}/bot_relay" roster
  if [[ "${HERMES_ALLOW_CROSS_INSTANCE_RELAY:-no}" == yes ]]; then
    if [[ -d "$relay_dir" ]]; then
      chown "$HERMES_UID:$HERMES_GID" -- "$relay_dir" 2>/dev/null || true
      chmod 755 -- "$relay_dir" 2>/dev/null || true
    fi
    info "Cross-instance relay ALLOWED (HERMES_ALLOW_CROSS_INSTANCE_RELAY=yes)."
    return 0
  fi
  install -d -m 755 -o "$HERMES_UID" -g "$HERMES_GID" "$relay_dir" 2>/dev/null \
    || mkdir -p -- "$relay_dir"
  roster="${relay_dir}/roster.json"
  printf '{"agents": [], "updated_at": %s}\n' "$(date +%s)" > "$roster"
  chmod 644 -- "$roster" 2>/dev/null || true
  if (( $(id -u) == 0 )); then
    chown "$HERMES_UID:$HERMES_GID" -- "$roster" 2>/dev/null || true
    chown root:root -- "$relay_dir" 2>/dev/null || true
  fi
  chmod 555 -- "$relay_dir" 2>/dev/null || true
  info "Cross-instance relay locked; in-instance Bot Mode unaffected."
}

isolate_profile_ui_meta() {
  local pf="${HERMES_DATA_DIR}/profiles/${HERMES_PROFILE}/profile.yaml"
  [[ -f "$pf" ]] && return 0
  mkdir -p -- "$(dirname -- "$pf")"
  {
    printf 'description: Profile %s. Scope is this node only.\n' "$HERMES_PROFILE"
    printf 'description_auto: false\n'
    printf 'ui_meta: {}\n'
  } > "$pf"
  chmod 644 -- "$pf"
  if (( $(id -u) == 0 )); then chown "$HERMES_UID:$HERMES_GID" "$pf"; fi
  info "Isolated profile ui_meta at ${pf}"
}

initialize_profile() {
  local api_key="$1" profile_env temp_env
  info "Preparing Hermes profile '$HERMES_PROFILE'..."
  if [[ -n "$HERMES_DISPLAY_NAME" ]]; then
    # One-persona instance: the requested name is a presentation-only display
    # name on the built-in root profile, so the instance holds exactly ONE
    # profile. Hermes reserves the root: its home IS the install root, its id
    # stays 'default' and its handle stays @hermes (bot_mode_probe._handle,
    # hermes_constants.profile_name_for_home). `profile rename default <name>`
    # is the supported way to relabel it (profiles.rename_profile ->
    # set_profile_display_name), and the display name also becomes a mention
    # alias, so @<name> addresses it (bot_mode_probe.local_alias_map).
    hermes_run profile rename default "$HERMES_DISPLAY_NAME"
  elif [[ "$HERMES_PROFILE" != default ]]; then
    hermes_run profile create "$HERMES_PROFILE"
    isolate_profile_ui_meta
  fi
  hermes_run profile use "$HERMES_PROFILE"
  profile_env="$(profile_env_path)"
  mkdir -p -- "$(dirname -- "$profile_env")"
  temp_env="$(mktemp "${profile_env}.tmp.XXXXXX")"
  if [[ -f "$profile_env" ]]; then
    awk '
      !/^(API_SERVER_ENABLED|API_SERVER_HOST|API_SERVER_PORT|API_SERVER_KEY|API_SERVER_CORS_ORIGINS)=/
    ' "$profile_env" > "$temp_env"
  fi
  {
    printf 'API_SERVER_ENABLED=true\n'
    printf 'API_SERVER_HOST=127.0.0.1\n'
    printf 'API_SERVER_PORT=8642\n'
    printf 'API_SERVER_KEY=%s\n' "$api_key"
    printf 'API_SERVER_CORS_ORIGINS=[]\n'
  } >> "$temp_env"
  chmod 600 "$temp_env"
  chown "$HERMES_UID:$HERMES_GID" "$temp_env" 2>/dev/null || true
  mv -f -- "$temp_env" "$profile_env"
}

run_setup() {
  info "Opening the official Hermes setup wizard for '$HERMES_PROFILE'."
  hermes_run -p "$HERMES_PROFILE" setup
}

configure_serve() {
  local failed=0 serve_status dashboard_url
  info "Configuring Tailnet-only HTTPS for dashboard and API..."
  compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock serve --bg --https=443 http://127.0.0.1:9119 || failed=1
  compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock serve --bg --https=8443 http://127.0.0.1:8642 || failed=1
  if (( failed )); then
    warn "Tailscale Serve needs attention (often HTTPS must be enabled for the tailnet)."
    warn "Follow the URL above if shown, then run: ./hermes-spawn.sh serve $TAILSCALE_HOSTNAME"
    return 1
  fi
  serve_status="$(compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock serve status)"
  printf '%s\n' "$serve_status"
  dashboard_url="$(printf '%s\n' "$serve_status" | grep -Eo 'https://[^[:space:]]+' | head -1 || true)"
  dashboard_url="${dashboard_url%/}"
  if [[ -n "$dashboard_url" ]]; then
    configure_dashboard_access "$dashboard_url"
  fi
}

configure_dashboard_access() {
  local public_url="$1" profile_env username password secret temp_env changed=0
  profile_env="$(profile_env_path)"
  mkdir -p -- "$(dirname -- "$profile_env")"
  username="$(read_control_value HERMES_DASHBOARD_BASIC_AUTH_USERNAME "$profile_env" 2>/dev/null || true)"
  password="$(read_control_value HERMES_DASHBOARD_BASIC_AUTH_PASSWORD "$profile_env" 2>/dev/null || true)"
  secret="$(read_control_value HERMES_DASHBOARD_BASIC_AUTH_SECRET "$profile_env" 2>/dev/null || true)"
  [[ -n "$username" ]] || { username=admin; changed=1; }
  [[ -n "$password" ]] || { password="$(openssl rand -hex 18)"; changed=1; }
  [[ -n "$secret" ]] || { secret="$(openssl rand -hex 32)"; changed=1; }
  if [[ "$(read_control_value HERMES_DASHBOARD_PUBLIC_URL "$profile_env" 2>/dev/null || true)" != "$public_url" ]]; then
    changed=1
  fi
  (( changed == 1 )) || return 0

  temp_env="$(mktemp "${profile_env}.tmp.XXXXXX")"
  if [[ -f "$profile_env" ]]; then
    awk '
      !/^(HERMES_DASHBOARD_PUBLIC_URL|HERMES_DASHBOARD_BASIC_AUTH_USERNAME|HERMES_DASHBOARD_BASIC_AUTH_PASSWORD|HERMES_DASHBOARD_BASIC_AUTH_SECRET)=/
    ' "$profile_env" > "$temp_env"
  fi
  {
    printf 'HERMES_DASHBOARD_PUBLIC_URL=%s\n' "$public_url"
    printf 'HERMES_DASHBOARD_BASIC_AUTH_USERNAME=%s\n' "$username"
    printf 'HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=%s\n' "$password"
    printf 'HERMES_DASHBOARD_BASIC_AUTH_SECRET=%s\n' "$secret"
  } >> "$temp_env"
  chmod 600 "$temp_env"
  chown "$HERMES_UID:$HERMES_GID" "$temp_env" 2>/dev/null || true
  mv -f -- "$temp_env" "$profile_env"
  if compose ps --status running --services | grep -Fqx hermes; then
    info "Restarting Hermes with Tailnet dashboard authentication..."
    compose restart hermes
  fi
}

# Publish a complete generation at /opt/data/fleet-skills. Never remove or
# overwrite a live skill: running readers may have open handles into it.
# An existing generation is kept outside the discovered fleet-skills tree.
seed_fleet_skills() {
  [[ -d "$SKILLS_DIR" ]] || { info "No fleet skills directory ($SKILLS_DIR); skipping."; return 0; }
  local src name dest target_dir stage history unsafe_entry installed=0 updated=0 skipped=0
  local -a managed=()
  target_dir="$HERMES_DATA_DIR/fleet-skills"
  # Outside /opt/data bind mount, so agents cannot enumerate rollback copies.
  history="$INSTANCE_DIR/.fleet-skills-history"
  [[ ! -L "$HERMES_DATA_DIR" && -d "$HERMES_DATA_DIR" ]] || die "Fleet skills data parent must be a real directory: $HERMES_DATA_DIR"
  [[ ! -L "$target_dir" && ( ! -e "$target_dir" || -d "$target_dir" ) ]] || die "Fleet skills target must be a real directory: $target_dir"
  [[ ! -L "$history" && ( ! -e "$history" || -d "$history" ) ]] || die "Fleet skills history must be a real directory: $history"
  if [[ -d "$history" ]]; then
    [[ $(stat -c %a "$history") == 700 && $(stat -c %u "$history") == $(id -u) ]] || die "Fleet skills history must be private and owned by the operator: $history"
  fi
  for src in "$SKILLS_DIR"/*; do
    [[ -e "$src" || -L "$src" ]] || continue
    [[ ! -L "$src" && -d "$src" ]] || die "Fleet skill source must be a real directory: $src"
    name="$(basename -- "$src")"
    [[ "$name" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]{0,63}$ ]] || die "Unsafe fleet skill name: $name"
    [[ -f "$src/SKILL.md" && ! -L "$src/SKILL.md" ]] || die "Fleet skill $src is missing a regular SKILL.md"
    # cp -a must never import an external file through a source symlink.
    unsafe_entry="$(find "$src" ! -type d ! -type f -print -quit)" || die "Could not inspect fleet skill source: $name"
    [[ -z "$unsafe_entry" ]] || die "Fleet skill source contains a link or special file: $name"
    managed+=("$name")
    dest="$target_dir/$name"
    [[ ! -L "$dest" && ( ! -e "$dest" || -d "$dest" ) ]] || die "Fleet skill must be a real directory: $dest"
    # A no-op must not endorse a modified live skill containing nested links
    # or devices, even if diff -r happens to dereference them as matching.
    if [[ -d "$dest" ]]; then
      unsafe_entry="$(find "$dest" ! -type d ! -type f -print -quit)" || die "Could not inspect installed fleet skill: $name"
      [[ -z "$unsafe_entry" ]] || die "Installed fleet skill contains a link or special file: $name"
    fi
    if [[ -d "$dest" ]] && diff -qr -- "$src" "$dest" >/dev/null 2>&1; then
      skipped=$((skipped + 1))
    elif [[ -d "$dest" ]]; then
      updated=$((updated + 1))
    else
      installed=$((installed + 1))
    fi
  done
  if (( installed == 0 && updated == 0 )); then
    info "Fleet skills: $installed installed, $updated refreshed, $skipped unchanged -> $target_dir"
    return 0
  fi

  # mktemp starts at 0700. No published skill is ever removed; on failure or
  # interruption this unique scratch tree is intentionally left for inspection.
  # The host-only instance dir keeps a post-exchange old generation private
  # even if SIGKILL arrives before archival. Never stage in agent bind mounts.
  [[ $(stat -c %a "$INSTANCE_DIR") == 700 && $(stat -c %u "$INSTANCE_DIR") == $(id -u) ]] || die "Instance directory must be operator-owned/private (0700) before staging fleet skills: $INSTANCE_DIR"
  [[ $(stat -c %d "$INSTANCE_DIR") == $(stat -c %d "$HERMES_DATA_DIR") ]] || die "Fleet skills stage and live path must share a filesystem; no changes made"
  stage="$(mktemp -d "$INSTANCE_DIR/.fleet-skills-stage-XXXXXXXX")"
  if [[ -d "$target_dir" ]]; then
    [[ $(stat -c %d "$target_dir") == $(stat -c %d "$stage") ]] || die "Fleet skills target is a separate filesystem; private stage retained at $stage"
    cp -a -- "$target_dir/." "$stage/"
  fi
  for name in "${managed[@]}"; do
    src="$SKILLS_DIR/$name"
    dest="$target_dir/$name"
    if [[ ! -d "$dest" ]] || ! diff -qr -- "$src" "$dest" >/dev/null 2>&1; then
      # This is only a throwaway copied tree, never the live/rollback copy.
      rm -rf -- "$stage/$name"
      cp -a -- "$src" "$stage/$name"
      find "$stage/$name" -type d -exec chmod 755 {} +
      find "$stage/$name" -type f -exec chmod 644 {} +
      if (( $(id -u) == 0 )); then chown -R "$HERMES_UID:$HERMES_GID" "$stage/$name"; fi
    fi
  done
  chmod 755 "$stage"
  if (( $(id -u) == 0 )); then chown "$HERMES_UID:$HERMES_GID" "$stage"; fi

  # Verify both managed repo skills AND each copied unmanaged top-level entry
  # before a single Linux renameat2 exchange (or NOREPLACE on first install).
  # Do not print file contents, diffs, or control.env on failure.
  python3 -B - "$target_dir" "$stage" "$SKILLS_DIR" "${managed[@]}" <<'PY'
import ctypes
import os
import stat
import sys

live, stage, source, *managed = sys.argv[1:]
managed = set(managed)

def fail(message):
    raise RuntimeError(message)

def equal_tree(a, b, links=False):
    def kind(entry):
        mode = entry.stat(follow_symlinks=False).st_mode
        if stat.S_ISDIR(mode): return "dir"
        if stat.S_ISREG(mode): return "file"
        if links and stat.S_ISLNK(mode): return "link"
        fail("unsafe file type")
    def equal_entry(x, y):
        t = kind(x)
        if t != kind(y): return False
        if t == "link": return os.readlink(x.path) == os.readlink(y.path)
        if t == "dir": return equal_tree(x.path, y.path, links)
        # O_NOFOLLOW keeps a concurrent symlink substitution from reading
        # files outside the tree. Compare bytes, not mtime or permissions.
        flags = os.O_RDONLY | os.O_NOFOLLOW | os.O_NONBLOCK
        with os.fdopen(os.open(x.path, flags), "rb") as left, os.fdopen(os.open(y.path, flags), "rb") as right:
            if not stat.S_ISREG(os.fstat(left.fileno()).st_mode) or not stat.S_ISREG(os.fstat(right.fileno()).st_mode):
                fail("unsafe file type during verification")
            while True:
                aa, bb = left.read(65536), right.read(65536)
                if aa != bb: return False
                if not aa: return True
    with os.scandir(a) as left, os.scandir(b) as right:
        la, lb = {e.name: e for e in left}, {e.name: e for e in right}
    return la.keys() == lb.keys() and all(equal_entry(la[n], lb[n]) for n in la)

try:
    if os.path.islink(live) or (os.path.lexists(live) and not os.path.isdir(live)):
        fail("unsafe destination")
    if os.path.islink(stage) or not os.path.isdir(stage):
        fail("unsafe stage")
    if os.stat(stage).st_dev != os.stat(os.path.dirname(live)).st_dev:
        fail("stage and live path are on different filesystems")
    actual = set(os.listdir(stage))
    if not managed <= actual: fail("missing staged skill")
    for name in managed:
        src_path, stage_path = os.path.join(source, name), os.path.join(stage, name)
        if not stat.S_ISDIR(os.lstat(src_path).st_mode) or not stat.S_ISDIR(os.lstat(stage_path).st_mode):
            fail("unsafe managed skill root")
        if not equal_tree(src_path, stage_path):
            fail("repo skill changed during staging")
    if os.path.isdir(live):
        if os.lstat(live).st_dev != os.stat(stage).st_dev:
            fail("live and stage are on different filesystems")
        original = set(os.listdir(live))
        if actual != (original | managed): fail("incomplete staged generation")
        for name in original - managed:
            if not equal_tree(os.path.join(live, name), os.path.join(stage, name), links=True):
                fail("unmanaged skill changed during staging")
    elif actual != managed:
        fail("unexpected staged entry")
    libc = ctypes.CDLL(None, use_errno=True)
    renameat2 = libc.renameat2
    renameat2.argtypes = (ctypes.c_int, ctypes.c_char_p, ctypes.c_int, ctypes.c_char_p, ctypes.c_uint)
    renameat2.restype = ctypes.c_int
    # Linux: exchange two non-empty directories atomically, or install
    # the first generation only if the name is still absent.
    flag = 2 if os.path.isdir(live) else 1  # RENAME_EXCHANGE / RENAME_NOREPLACE
    rc = renameat2(-100, os.fsencode(stage), -100, os.fsencode(live), flag)  # AT_FDCWD
    if rc: fail("Linux atomic rename unavailable or failed (errno %d)" % ctypes.get_errno())
except (OSError, RuntimeError, AttributeError) as exc:
    # Do not echo source names, contents, or exception paths (potential secrets).
    print("Fleet skills: stage verification or atomic publish failed; live tree is unchanged; private stage retained.", file=sys.stderr)
    sys.exit(1)
PY
  # stage now names the exact previous live generation; never delete it.
  if [[ -d "$stage" ]]; then
    if [[ ! -e "$history" ]]; then
      install -d -m 700 "$history"
    fi
    [[ ! -L "$history" && -d "$history" ]] || die "Fleet skills history is not a real directory: $history; old generation remains at $stage"
    [[ $(stat -c %a "$history") == 700 && $(stat -c %u "$history") == $(id -u) ]] || die "Fleet skills history is not operator-owned/private; old generation remains at $stage"
    [[ ! -e "$history/${stage##*/}" && ! -L "$history/${stage##*/}" ]] || die "Fleet skills history entry already exists; old generation remains at $stage"
    mv -T -- "$stage" "$history/${stage##*/}" || die "Could not archive old generation; it remains at $stage"
  fi
  info "Fleet skills: $installed installed, $updated refreshed, $skipped unchanged -> $target_dir"
}

install_skills_command() {
  seed_fleet_skills
}

# The declaration precedes any native profile creation. It never certifies crew
# formation or changes the instance launch profile.
declare_formation_intent() {
  python3 -B "$SCRIPT_DIR/lib/formation_status.py" declare --instance-dir "$1" --mode "$2" \
    || die "Cannot declare formation intent for $1; native creation was not attempted."
}

# Native creation never clones another persona. Do not select the new profile:
# the instance launch profile and existing per-profile state stay unchanged.
add_persona_command() {
  [[ $# == 1 ]] || die "Usage: add-persona instance name"
  local name="$1"
  validate_profile "$name" && [[ "$name" != default ]] || die "Invalid/reserved persona name: $name"
  [[ ! -e "$HERMES_DATA_DIR/profiles/$name" && ! -L "$HERMES_DATA_DIR/profiles/$name" ]] || die "Persona already exists: $name"
  compose ps --status running --services | grep -Fqx hermes || die "Instance must be running; start it explicitly first."
  # The operator's add-persona invocation chooses crew. In particular, upgrade
  # a prior solo intent before the first named native profile exists.
  declare_formation_intent "$INSTANCE_DIR" crew
  # Native create is clean by default; never pass --clone or copy profile data.
  compose exec -T hermes hermes profile create "$name"
  [[ -d "$HERMES_DATA_DIR/profiles/$name" ]] || die "Native profile create did not create $name"
  ( HERMES_PROFILE="$name"; isolate_profile_ui_meta )
  seed_fleet_skills
  apply_stack_core yes no
  stack_status_core --offline
  # Installed Hermes supports hot profile reconciliation. Never kill other
  # personas/sessions to add one. Older gateways fail with an actionable error.
  compose exec -T -e HERMES_HOME=/opt/data -w /opt/hermes hermes python3 -B -c '
import sys
from hermes_cli.gateway_multiplex_served import notify_multiplexer_profiles_changed
name = sys.argv[1]
served = notify_multiplexer_profiles_changed(name)
if served is None or name not in served:
    raise SystemExit("Persona created but gateway did not confirm hot serving. Enable multiplexing, then schedule a gateway restart when existing sessions are idle.")
print("Native gateway hot-serving verified: " + name)' "$name"
  audit_isolation_command
  verify_persona_fleet_skills "$name"
  info "Persona $name created and verified (launch profile unchanged)."
}

verify_persona_fleet_skills() {
  local name="$1"
  # Use the installed Hermes discovery and read paths, not merely file presence.
  # HERMES_HOME is set before import so native config uses the new persona.
  compose exec -T -e "HERMES_HOME=/opt/data/profiles/$name" -w /opt/hermes hermes python3 -B -c '
import json
from pathlib import Path
import yaml
from tools.skills_tool import _find_all_skills, skill_view
visible = {s["name"] for s in _find_all_skills()}
for source in sorted(Path("/opt/data/fleet-skills").glob("*/SKILL.md")):
    text = source.read_text()
    front = yaml.safe_load(text.split("---", 2)[1]) if text.startswith("---") else {}
    name = (front or {}).get("name", source.parent.name)
    if name not in visible:
        raise SystemExit("Fleet skill not visible through native discovery: " + name)
    result = json.loads(skill_view(name, preprocess=False))
    if result.get("success") is False or result.get("error"):
        raise SystemExit("Fleet skill cannot be read through native skill_view: " + name)
    resolved = result.get("_source_path")
    if not resolved or Path(resolved).read_bytes() != source.read_bytes():
        raise SystemExit("Fleet skill is shadowed or native source cannot be verified: " + name)
print("Native fleet skill discovery/read verified")'
}

spawn_instance() {
  local suggested_host cpu_total memory_total cpu_default memory_default
  local authkey api_key dir formation_mode
  host_preflight
  install -d -m 700 "$INSTANCES_DIR"

  suggested_host="hermes-$(hostname -s 2>/dev/null | tr '[:upper:]_' '[:lower:]-' | tr -cd 'a-z0-9-' | cut -c1-50)"
  TAILSCALE_HOSTNAME="$(prompt_default 'Tailscale hostname / instance name' "$suggested_host")"
  TAILSCALE_HOSTNAME="${TAILSCALE_HOSTNAME,,}"
  validate_hostname "$TAILSCALE_HOSTNAME" || die "Use 1-63 lowercase letters, digits, or interior hyphens."
  dir="$INSTANCES_DIR/$TAILSCALE_HOSTNAME"
  [[ ! -e "$dir" ]] || die "Instance already exists: $TAILSCALE_HOSTNAME"

  # A name here labels the instance's single built-in profile; it does NOT add a
  # second one. Hermes always has a root profile whose id is 'default', so
  # creating a named profile would leave 'default' beside it. Answer 'yes' to
  # the multi-persona question only when this instance really runs a crew.
  local persona_name
  persona_name="$(prompt_default 'Name for this instance'"'"'s agent' 'default')"
  validate_profile "$persona_name" || die "Invalid/reserved name (letters, digits, _ and -; max 48)."
  HERMES_PROFILE=default
  HERMES_DISPLAY_NAME=""
  formation_mode=solo
  if [[ "$persona_name" != default ]]; then
    if confirm "Run several named personas on this instance (keeps a separate '${persona_name}' profile alongside 'default')?" no; then
      HERMES_PROFILE="$persona_name"
      formation_mode=crew
    else
      HERMES_DISPLAY_NAME="$persona_name"
    fi
  fi

  cpu_total="$(host_cpu_count)"
  memory_total="$(host_memory_mib)"
  cpu_default="$(default_cpu_limit "$cpu_total")"
  memory_default="$(default_memory_limit "$memory_total")"
  HERMES_CPUS="$(prompt_default 'Hermes CPU limit' "$cpu_default")"
  validate_cpu "$HERMES_CPUS" || die "CPU must be a positive number such as 4 or 7.5."
  HERMES_MEMORY="$(prompt_default 'Hermes memory limit' "$memory_default")"
  validate_memory "$HERMES_MEMORY" || die "Memory must look like 8g or 16384m."

  printf '\nCreate a one-off or reusable auth key at:\n'
  printf '  https://login.tailscale.com/admin/settings/keys\n'
  authkey="$(tailscale_authkey 'Tailscale auth key: ')"

  COMPOSE_PROJECT_NAME="hermes-${TAILSCALE_HOSTNAME}"
  TAILSCALE_STATE_DIR="$dir/tailscale-state"
  TAILSCALE_AUTHKEY_FILE="$dir/tailscale-authkey"
  TS_AUTHKEY_SPEC="file:/run/secrets/tailscale-authkey"
  HERMES_DATA_DIR="$dir/hermes-data"
  if (( $(id -u) == 0 )); then HERMES_UID=10000; HERMES_GID=10000
  else HERMES_UID="$(id -u)"; HERMES_GID="$(id -g)"; fi
  HERMES_MEMORY_RESERVATION=1g
  HERMES_SHM_SIZE=1g
  HERMES_PIDS_LIMIT=4096
  HERMES_IMAGE=nousresearch/hermes-agent:latest
  TAILSCALE_IMAGE=tailscale/tailscale:latest

  install -d -m 700 "$dir" "$TAILSCALE_STATE_DIR" "$HERMES_DATA_DIR"
  # Record the wizard's explicit solo/crew choice before compose or gateway setup.
  declare_formation_intent "$dir" "$formation_mode"
  printf '%s' "$authkey" > "$TAILSCALE_AUTHKEY_FILE"
  chmod 600 "$TAILSCALE_AUTHKEY_FILE"
  if (( $(id -u) == 0 )); then chown -R 10000:10000 "$HERMES_DATA_DIR"; fi
  write_control_file "$dir/control.env"
  collect_host_info "$dir/system-info.txt"
  api_key="$(openssl rand -hex 32)"

  INSTANCE_DIR="$dir"
  export INSTANCE_DIR
  load_instance "$dir"
  info "Pulling the official container images..."
  compose pull
  compose config --quiet
  compose up -d tailscale
  wait_for_tailscale
  initialize_profile "$api_key"
  seed_fleet_skills
  lock_bot_relay
  # Fleet policy is required provisioning, not an optional setup preference.
  apply_stack_core yes no
  if confirm 'Run the Hermes provider/channel setup wizard now?' yes; then
    run_setup
  else
    warn "Hermes will start, but it needs setup before it can answer. Run: ./hermes-spawn.sh setup $TAILSCALE_HOSTNAME"
  fi
  # Setup can rewrite config and timeout env values. Fleet policy wins last.
  apply_stack_core yes no
  stack_status_core --offline
  compose up -d hermes
  configure_serve || true

  # Every spawn gets webui + serve routes by default, and host-side autostart.
  webui_ensure_instance apply || warn "webui setup failed — rerun: ./hermes-spawn.sh webui-ensure $TAILSCALE_HOSTNAME"
  if command -v systemctl >/dev/null 2>&1 && (( $(id -u) == 0 )); then
    if ! systemctl is-enabled "$FLEET_SUPERVISOR_UNIT_NAME" >/dev/null 2>&1; then
      if confirm 'Enable the fleet supervisor (webui + serve routes auto-restart)?' yes; then
        supervisor_command install || warn "supervisor install failed — rerun: ./hermes-spawn.sh supervisor install"
      fi
    fi
  elif ! command -v systemctl >/dev/null 2>&1; then
    info "No systemd — start scripts/fleet-supervisor.sh manually (keep-alive task)."
  fi

  printf '\nInstance %s is running.\n' "$TAILSCALE_HOSTNAME"
  credentials_command
  printf '\nManage it with: ./hermes-spawn.sh status %s\n' "$TAILSCALE_HOSTNAME"
}

ensure_running() {
  compose up -d
  wait_for_tailscale
  # Heal webui + routes for already-vendored instances (cheap; new deployments
  # happen via webui-ensure apply / spawn).
  if [[ -f "$HERMES_DATA_DIR/hermes-webui/ctl.sh" ]]; then
    webui_ensure_instance launch || true
  elif webui_enabled && [[ -n "$WEBUI_SETUP_LIB" ]]; then
    info "webui not deployed for $TAILSCALE_HOSTNAME — run: ./hermes-spawn.sh webui-ensure $TAILSCALE_HOSTNAME"
  fi
}

setup_command() {
  ensure_running
  if compose ps --status running --services | grep -Fqx hermes; then
    compose exec hermes hermes -p "$HERMES_PROFILE" setup
    compose restart hermes
  else
    run_setup
    compose up -d hermes
  fi
}

# ------------------------------------------------------------- fleet stack
# One-endpoint model stack (see stack-defaults.yaml). deploy.sh records the
# endpoint in .model-endpoint (mode 0600); every apply refreshes control.env.
STACK_DEFAULTS_FILE="$SCRIPT_DIR/stack-defaults.yaml"
stack_status_command() {
  [[ $# == 0 ]] || die "stack-status takes no options"
  stack_status_core
}

webui_enabled() {
  [[ -f "$STACK_DEFAULTS_FILE" ]] && grep -qE '^  enabled:.*(true|yes|1)' "$STACK_DEFAULTS_FILE" 2>/dev/null
}

# Idempotent webui deploy+launch+routes via lib/webui_setup.py.
#  $1 = "apply" (clone+deploy+env+launch+routes) or "launch" (start+routes only)
webui_ensure_instance() {
  [[ -f "$WEBUI_SETUP_LIB" ]] || { warn "webui support files missing ($WEBUI_SETUP_LIB)"; return 1; }
  webui_enabled || { info "webui disabled in stack-defaults.yaml — skipping."; return 0; }
  python3 "$WEBUI_SETUP_LIB" "${1:-apply}" \
    --home "$HERMES_DATA_DIR" \
    --instance "$TAILSCALE_HOSTNAME" \
    --control "$INSTANCE_DIR/control.env" \
    --defaults "$STACK_DEFAULTS_FILE" \
    --launch-profile "$HERMES_PROFILE" \
    --uid "$HERMES_UID" --gid "$HERMES_GID"
}

webui_ensure_command() { webui_ensure_instance apply; }
webui_status_command() {
  if [[ ! -d "$HERMES_DATA_DIR/hermes-webui" ]]; then
    echo "webui not deployed for $TAILSCALE_HOSTNAME (run: ./hermes-spawn.sh webui-ensure $TAILSCALE_HOSTNAME)"
    return 0
  fi
  python3 "$WEBUI_SETUP_LIB" status \
    --home "$HERMES_DATA_DIR" \
    --instance "$TAILSCALE_HOSTNAME" \
    --control "$INSTANCE_DIR/control.env" \
    --defaults "$STACK_DEFAULTS_FILE"
}

WEBUI_SETUP_LIB="$SCRIPT_DIR/lib/webui_setup.py"
STACK_APPLY_LIB="$SCRIPT_DIR/lib/apply_stack.py"
FLEET_SUPERVISOR_SCRIPT="$SCRIPT_DIR/scripts/fleet-supervisor.sh"
FLEET_SUPERVISOR_UNIT_SRC="$SCRIPT_DIR/systemd/hermes-fleet-supervisor.service"
FLEET_SUPERVISOR_UNIT_NAME="hermes-fleet-supervisor.service"
STACK_ENDPOINT_FILE="$SCRIPT_DIR/.model-endpoint"

model_stack_resolve() {
  # Endpoint + key from control.env, then .model-endpoint, then host Tailscale.
  if [[ -z "${MODEL_STACK_BASE_URL:-}" && -r "$STACK_ENDPOINT_FILE" ]]; then
    # Read endpoint data only; audits must never evaluate shell from this file.
    MODEL_STACK_BASE_URL="$(read_control_value MODEL_STACK_BASE_URL "$STACK_ENDPOINT_FILE")"
    if [[ -z "${MODEL_STACK_API_KEY:-}" ]]; then
      MODEL_STACK_API_KEY="$(read_control_value MODEL_STACK_API_KEY "$STACK_ENDPOINT_FILE")"
    fi
  fi
  if [[ -z "${MODEL_STACK_BASE_URL:-}" ]]; then
    local tsip=""
    tsip="$(tailscale ip -4 2>/dev/null | head -1 || true)"
    [[ -n "$tsip" ]] && MODEL_STACK_BASE_URL="http://${tsip}:8317/v1"
  fi
  MODEL_STACK_API_KEY="${MODEL_STACK_API_KEY:-ccs-internal-managed}"
  export MODEL_STACK_BASE_URL MODEL_STACK_API_KEY
}

model_stack_reachable() {
  [[ -n "${MODEL_STACK_BASE_URL:-}" ]] || return 1
  curl -fsS --max-time 10 -o /dev/null \
    -H "Authorization: Bearer ${MODEL_STACK_API_KEY}" \
    "${MODEL_STACK_BASE_URL%/}/models" 2>/dev/null
}

model_stack_persist() {
  local control="$INSTANCE_DIR/control.env" tmp
  tmp="$(mktemp "${control}.tmp.XXXXXX")"
  grep -v '^MODEL_STACK_' "$control" > "$tmp" || true
  printf 'MODEL_STACK_BASE_URL=%s\nMODEL_STACK_API_KEY=%s\n' \
    "$MODEL_STACK_BASE_URL" "$MODEL_STACK_API_KEY" >> "$tmp"
  chmod 600 "$tmp"
  mv -f -- "$tmp" "$control"
}

model_stack_require() {
  [[ -n "${MODEL_STACK_BASE_URL:-}" ]] || die "No model endpoint known. Run: ./deploy.sh on this host first."
  if [[ -t 0 ]]; then
    curl -fsS --max-time 10 -o /dev/null \
      -H "Authorization: Bearer ${MODEL_STACK_API_KEY}" \
      "${MODEL_STACK_BASE_URL%/}/models" || warn "Model endpoint $MODEL_STACK_BASE_URL is not reachable."
  fi
}

require_stack_tools() {
  require_command python3
  python3 -c 'import yaml' >/dev/null 2>&1 || die "PyYAML is required on this host (e.g. python3-yaml)."
  [[ -f "$STACK_DEFAULTS_FILE" ]] || die "Missing $STACK_DEFAULTS_FILE"
  [[ -f "$STACK_APPLY_LIB" ]] || die "Missing $STACK_APPLY_LIB"
}

stack_python() {
  MODEL_STACK_ENDPOINT="$MODEL_STACK_BASE_URL" MODEL_STACK_API_KEY="$MODEL_STACK_API_KEY" \
    python3 "$STACK_APPLY_LIB" "$@"
}

# An instance may opt out of the fleet stack defaults by setting
# STACK_MANAGED=no in control.env preserves model/provider/timeout pins.
# Common fleet policy still applies and is audited for these instances.
stack_opt_out() {
  local v
  v="$(read_control_value STACK_MANAGED "$INSTANCE_DIR/control.env" 2>/dev/null || true)"
  [[ "$v" == no || "$v" == false || "$v" == 0 ]]
}

apply_stack_core() {
  # Apply to the resolved, loaded instance. $1 = yes (skip prompts) / no.
  local auto="${1:-no}" restart_choice="${2:-prompt}"
  local policy_args=()
  if stack_opt_out; then
    info "$TAILSCALE_HOSTNAME: STACK_MANAGED=no; applying common policy, preserving model stack."
    policy_args=(--policy-only)
  fi
  require_stack_tools
  if (( ${#policy_args[@]} == 0 )); then
    model_stack_resolve
    [[ -n "${MODEL_STACK_BASE_URL:-}" ]] || die "No model endpoint known. Run ./deploy.sh on this host first."
    if ! model_stack_reachable; then
      warn "Model endpoint $MODEL_STACK_BASE_URL is not reachable (CLIProxy down or wrong key)."
      warn "Run ./deploy.sh on this host first, then retry."
      if [[ "$auto" == yes ]]; then
        die "Endpoint unreachable; aborting apply-stack."
      else
        confirm 'Apply the stack anyway (Hermes will fail until the endpoint is back)?' no || die "Aborted."
      fi
    fi
  fi
  seed_fleet_skills
  local apply_out
  apply_out="$(stack_python apply "${policy_args[@]}" \
    --home "$HERMES_DATA_DIR" \
    --launch-profile "$HERMES_PROFILE" \
    --base-url "$MODEL_STACK_BASE_URL" \
    --api-key "$MODEL_STACK_API_KEY" \
    --defaults "$STACK_DEFAULTS_FILE" \
    --uid "$HERMES_UID" --gid "$HERMES_GID")"
  printf '%s\n' "$apply_out"
  local changes=0
  if printf '%s\n' "$apply_out" | grep -q '^  wrote '; then changes=1; fi
  if (( ${#policy_args[@]} == 0 )); then model_stack_persist; fi
  if (( changes == 1 )); then
    stack_status_core --offline >/dev/null 2>&1 || warn "Post-apply audit found drift; run: ./hermes-spawn.sh stack-status $TAILSCALE_HOSTNAME"
  fi
  if (( changes == 0 )); then
    info "No drift on disk — everything already matches stack-defaults.yaml."
    return 0
  fi
  if compose ps --status running --services 2>/dev/null | grep -Fqx hermes; then
    local do_restart=no
    if [[ "$restart_choice" == yes ]]; then do_restart=yes
    elif [[ "$restart_choice" == no ]]; then do_restart=no
    elif [[ -t 0 ]]; then
      confirm 'Hermes is running. Restart it now so the new stack takes effect?' yes && do_restart=yes
    fi
    if [[ "$do_restart" == yes ]]; then
      info "Restarting Hermes to activate the stack..."
      compose restart hermes
    else
      warn "Hermes is still on the old config. Run: ./hermes-spawn.sh restart $TAILSCALE_HOSTNAME"
    fi
  fi
}

stack_status_core() {
  # Audit only (read-only). $1 = optional --offline.
  local policy_args=()
  if stack_opt_out; then
    policy_args=(--policy-only)
  fi
  require_stack_tools
  local reachable=offline python_rc=0
  if (( ${#policy_args[@]} == 0 )); then
    model_stack_resolve
    [[ -n "${MODEL_STACK_BASE_URL:-}" ]] || die "No model endpoint known. Run ./deploy.sh on this host first."
    if [[ "${1:-}" != --offline ]]; then
      if model_stack_reachable; then reachable=yes; else reachable=no; fi
    fi
  fi
  stack_python status "${policy_args[@]}" \
    --home "$HERMES_DATA_DIR" \
    --launch-profile "$HERMES_PROFILE" \
    --base-url "$MODEL_STACK_BASE_URL" \
    --api-key "$MODEL_STACK_API_KEY" \
    --defaults "$STACK_DEFAULTS_FILE" ${1:+--quiet} || python_rc=$?
  if [[ "$reachable" == no ]]; then
    warn "Model endpoint $MODEL_STACK_BASE_URL unreachable (no live health check)."
    return 2
  elif [[ "$reachable" == yes ]]; then
    info "Model endpoint OK: $MODEL_STACK_BASE_URL"
  fi
  return $python_rc
}

apply_stack_command() {
  local auto=no restart_choice=prompt
  while [[ $# -gt 0 ]]; do
    case "$1" in
      -y|--yes) auto=yes ;;
      --restart) restart_choice=yes ;;
      --no-restart) restart_choice=no ;;
      *) die "Unknown apply-stack option: $1" ;;
    esac
    shift
  done
  apply_stack_core "$auto" "$restart_choice"
}

stack_status_command() {
  [[ $# == 0 ]] || die "stack-status takes no options"
  stack_status_core
}

credentials_command() {
  local profile_env key serve_status dashboard_url api_url dashboard_user dashboard_password
  profile_env="$(profile_env_path)"
  key="$(awk -F= '$1 == "API_SERVER_KEY" {sub(/^[^=]*=/, ""); print; exit}' "$profile_env" 2>/dev/null || true)"
  dashboard_user="$(read_control_value HERMES_DASHBOARD_BASIC_AUTH_USERNAME "$profile_env" 2>/dev/null || true)"
  # config.yaml's dashboard.basic_auth.username WINS over the env var (operators
  # rename the account there), so printing the env
  # value alone hands out a username that 401s. Prefer the live config.
  local cfg_user
  cfg_user="$(sed -n '/^  basic_auth:/,/^  [^ ]/p' "$HERMES_DATA_DIR/config.yaml" 2>/dev/null \
    | sed -n 's/^[[:space:]]*username:[[:space:]]*//p' | head -1)"
  [[ -n "$cfg_user" ]] && dashboard_user="$cfg_user"
  dashboard_password="$(read_control_value HERMES_DASHBOARD_BASIC_AUTH_PASSWORD "$profile_env" 2>/dev/null || true)"
  serve_status="$(compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock serve status 2>/dev/null || true)"
  dashboard_url="$(printf '%s\n' "$serve_status" | grep -Eo 'https://[^[:space:]]+' | head -1 || true)"
  api_url="$(printf '%s\n' "$serve_status" | grep -Eo 'https://[^[:space:]]+:8443' | head -1 || true)"
  printf 'Dashboard: %s\n' "${dashboard_url:-Tailnet HTTPS not configured; run the serve command}"
  if [[ -n "$api_url" ]]; then
    printf 'API base:  %s/v1\n' "$api_url"
  else
    printf 'API base:  Tailnet HTTPS not configured; run the serve command\n'
  fi
  printf 'API key:   %s\n' "${key:-not configured}"
  printf '\n'
  printf 'Dashboard login (browser at the URL above, and Hermex "Bot Mode"):\n'
  printf '  user:    %s\n' "${dashboard_user:-not configured}"
  # When config.yaml carries its own basic_auth block, THAT is what the dashboard
  # verifies against (password stored only as a scrypt hash). The env password is
  # then stale, so flag it instead of printing a credential that 401s.
  if [[ -n "$cfg_user" ]] && grep -q '^    password_hash:' "$HERMES_DATA_DIR/config.yaml" 2>/dev/null; then
    printf '  pass:    (set by you in config.yaml; not recoverable - stored as a hash)\n'
  else
    printf '  pass:    %s\n' "${dashboard_password:-not configured}"
  fi
  printf '\n'
  # The webui has its OWN password (password-only, no username). The Hermex iOS
  # app strips any path from the URL it is given, so it must use the :8788
  # port-origin, not /webui. Reported here because handing out the dashboard
  # password for Hermes Mobile is the single most common setup mistake.
  local webui_pass webui_origin webui_port
  # Same single source of truth the supervisor and lib/webui_setup.py use.
  webui_port="$(sed -n 's/^[[:space:]]*serve_port:[[:space:]]*\([0-9][0-9]*\).*/\1/p' "$STACK_DEFAULTS_FILE" 2>/dev/null | head -1)"
  [[ -n "$webui_port" ]] || webui_port=8788
  webui_pass="$(read_control_value HERMES_WEBUI_PASSWORD "$HERMES_DATA_DIR/hermes-webui/.env" 2>/dev/null || true)"
  webui_origin="$(printf '%s\n' "$serve_status" | grep -Eo "https://[^[:space:]]+:${webui_port}" | head -1 || true)"
  printf 'Hermes Mobile / Hermex app  (password only, no username):\n'
  if [[ -n "$webui_origin" ]]; then
    printf '  server:  %s\n' "$webui_origin"
  else
    printf '  server:  %s:%s\n' "${dashboard_url:-<tailnet URL>}" "$webui_port"
  fi
  printf '  pass:    %s\n' "${webui_pass:-not deployed; run: ./hermes-spawn.sh webui-ensure $TAILSCALE_HOSTNAME}"
  printf '  browser: %s/webui  (same password)\n' "${dashboard_url:-<tailnet URL>}"
  printf '\n'
  if [[ -n "${HERMES_DISPLAY_NAME:-}" ]]; then
    printf 'Profile:   %s (%s)  handle: @hermes or @%s\n' \
      "$HERMES_DISPLAY_NAME" "$HERMES_PROFILE" "${HERMES_DISPLAY_NAME,,}"
  else
    printf 'Profile:   %s\n' "$HERMES_PROFILE"
  fi
}

# ---------------------------------------------------------------- supervisor
# Host-side fleet supervisor: one systemd unit keeps webui + serve routes alive
# across container restarts for every running instance. Operator-approved default.
supervisor_command() {
  local sub="${1:-status}"
  if ! command -v systemctl >/dev/null 2>&1; then
    warn "No systemd on this host. Install manually:"
    printf '  macOS launchd equivalent: run scripts/fleet-supervisor.sh as a keep-alive agent.\n'
    return 1
  fi
  case "$sub" in
    install)
      require_command sed
      local unit_dest="/etc/systemd/system/$FLEET_SUPERVISOR_UNIT_NAME"
      [[ -f "$FLEET_SUPERVISOR_UNIT_SRC" ]] || die "missing $FLEET_SUPERVISOR_UNIT_SRC"
      if (( $(id -u) != 0 )); then die "supervisor install needs root (unit write + systemctl)."; fi
      sed "s|@HERMES_SPAWNING_REPO@|$SCRIPT_DIR|g" "$FLEET_SUPERVISOR_UNIT_SRC" > "$unit_dest"
      systemctl daemon-reload
      systemctl enable --now "$FLEET_SUPERVISOR_UNIT_NAME"
      systemctl --no-pager --lines=5 status "$FLEET_SUPERVISOR_UNIT_NAME" || true
      ;;
    enable)  systemctl enable --now "$FLEET_SUPERVISOR_UNIT_NAME" ;;
    disable) systemctl disable --now "$FLEET_SUPERVISOR_UNIT_NAME" ;;
    status)
      systemctl --no-pager --lines=10 status "$FLEET_SUPERVISOR_UNIT_NAME" || true
      [[ -f "$INSTANCES_DIR/.supervisor.log" ]] && tail -n 15 "$INSTANCES_DIR/.supervisor.log" || true
      ;;
    *) die "supervisor subcommands: install enable disable status" ;;
  esac
}

reauth_command() {
  local authkey
  printf 'Create a fresh one-off or reusable auth key at:\n'
  printf '  https://login.tailscale.com/admin/settings/keys\n'
  authkey="$(tailscale_authkey 'New Tailscale auth key: ')"
  printf '%s' "$authkey" > "$TAILSCALE_AUTHKEY_FILE"
  chmod 600 "$TAILSCALE_AUTHKEY_FILE"
  compose up -d --force-recreate tailscale hermes
  wait_for_tailscale
  configure_serve || true
}

# ------------------------------------------------------------------ remove
# Permanently delete an instance: containers, data directory, and tailnet
# membership. Everything under instances/<name>/ is destroyed, so the default
# is to archive it first. Deleting the directory also drops the instance from
# the fleet supervisor's sweep, because that loop iterates instances/*/.
#
# The Tailscale DEVICE RECORD is server-side state and is NOT removed by
# deleting local files: `tailscale logout` retires the node key, but the
# machine keeps showing (offline) in the admin console until an operator or an
# API key deletes it. A surviving record holds the hostname, so the next spawn
# of that name is renamed <name>-1 by the coordination server.
remove_command() {
  local keep_data=no auto=no archive="" answer tar_rc=0
  while [[ $# -gt 0 ]]; do
    case "$1" in
      -y|--yes) auto=yes ;;
      --keep-data) keep_data=yes ;;
      *) die "Unknown remove option: $1" ;;
    esac
    shift
  done

  printf 'This permanently removes instance %s:\n' "$TAILSCALE_HOSTNAME"
  printf '  - stops and deletes its containers\n'
  printf '  - logs the node out of the tailnet\n'
  if [[ "$keep_data" == yes ]]; then
    printf '  - KEEPS %s (--keep-data)\n' "$INSTANCE_DIR"
  else
    printf '  - archives, then deletes %s\n' "$INSTANCE_DIR"
    printf '    (profiles, memories, skills, databases, credentials)\n'
  fi

  if [[ "$auto" != yes ]]; then
    # Typing the hostname beats y/N here: an operator fanning out several
    # removals cannot confirm the wrong instance by reflex.
    read -r -p "Type the instance name to confirm: " answer
    [[ "$answer" == "$TAILSCALE_HOSTNAME" ]] || die "Name did not match; nothing was removed."
  fi

  # Log out while the container still exists, so the node key is retired
  # rather than left dangling. Best effort: a stopped or broken instance must
  # still be removable.
  if compose ps --status running --services 2>/dev/null | grep -qx tailscale; then
    info "Logging $TAILSCALE_HOSTNAME out of the tailnet..."
    compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock logout >/dev/null 2>&1 \
      || warn "Logout failed; delete the device in the admin console."
  fi

  # Containers go down BEFORE the archive. A running instance rewrites
  # databases and holds gateway sockets open while tar reads, which yields a
  # torn snapshot ("file changed as we read it") rather than a restorable one.
  info "Stopping and removing containers..."
  compose down --remove-orphans --timeout 30 || warn "compose down reported an error; continuing."

  # Archive the quiesced directory. tar keeps ownership and modes that a plain
  # copy would flatten.
  if [[ "$keep_data" != yes ]]; then
    mkdir -p "$SCRIPT_DIR/.cache"
    archive="$SCRIPT_DIR/.cache/${TAILSCALE_HOSTNAME}-removed-$(date -u +%Y%m%d-%H%M%S).tar.gz"
    info "Archiving $INSTANCE_DIR ..."
    # tar exits 1 for WARNINGS and 2 for fatal errors. Unix sockets under
    # hermes-data are always "socket ignored" warnings, so treating 1 as
    # failure would make every removal look broken. Only 2 is fatal, and the
    # archive is then verified by listing it back.
    # `|| tar_rc=$?` is required: under `set -e` a bare tar returning 1 would
    # abort the script before the exit code could be inspected.
    tar_rc=0
    tar -C "$INSTANCES_DIR" -czf "$archive" "$TAILSCALE_HOSTNAME" 2>"$archive.log" || tar_rc=$?
    if (( tar_rc > 1 )) || ! tar -tzf "$archive" >/dev/null 2>&1; then
      sed 's/^/  /' "$archive.log" >&2 || true
      rm -f "$archive" "$archive.log"
      die "Archive failed (tar exit $tar_rc); $INSTANCE_DIR was NOT deleted. Containers are stopped; restart with: ./hermes-spawn.sh start $TAILSCALE_HOSTNAME"
    fi
    rm -f "$archive.log"
    chmod 600 "$archive"
    info "Archived to $archive"
  fi

  if [[ "$keep_data" != yes ]]; then
    # Release the lock we hold on this directory before deleting it, otherwise
    # the open descriptor points at an unlinked file for the rest of the run.
    exec 9>&-
    rm -rf -- "$INSTANCE_DIR"
    info "Deleted $INSTANCE_DIR"
  else
    info "Kept $INSTANCE_DIR (containers removed; re-create with start)"
  fi

  printf '\n'
  info "Instance $TAILSCALE_HOSTNAME removed."
  printf 'One manual step remains. The tailnet still lists this machine, and\n'
  printf 'while it does, the name %s stays taken:\n' "$TAILSCALE_HOSTNAME"
  printf '  https://login.tailscale.com/admin/machines\n'
  printf 'Find %s, open the ... menu, choose Delete. Re-spawning before that\n' "$TAILSCALE_HOSTNAME"
  printf 'gets you %s-1 instead.\n' "$TAILSCALE_HOSTNAME"
}

# ------------------------------------------------------------------ fan-out
# Run one command across every instance, sequentially. Re-invokes this script
# per instance so each run keeps its own validation, lock, and env isolation.
# Interactive commands are refused: they need one dedicated terminal.
# remove/delete are excluded: a fleet-wide accidental wipe must not be one
# keystroke away, and each removal needs its own typed confirmation.
FANOUT_FORBIDDEN=" chat shell logs setup config spawn create reauth remove delete add-persona "

run_all_command() {
  local command="$1"; shift
  local dir name rc=0 failed=() ran=0

  if [[ "$FANOUT_FORBIDDEN" == *" $command "* ]]; then
    die "Command '$command' is interactive; run it per instance (no 'all')."
  fi

  # Collect the list FIRST. Running the child inside a `while read` loop lets it
  # consume the loop's stdin, which silently truncated the fan-out to one instance.
  local dirs=()
  while IFS= read -r dir; do
    [[ -n "$dir" ]] || continue
    dirs+=("$dir")
  done < <(instance_dirs)

  for dir in "${dirs[@]}"; do
    name="$(basename -- "$dir")"
    ran=$((ran + 1))
    printf '\n\033[1;34m===> %s: %s\033[0m\n' "$name" "$command"
    # </dev/null so no child can steal the terminal or block on a prompt.
    if "$SCRIPT_DIR/hermes-spawn.sh" "$command" "$name" "$@" </dev/null; then
      :
    else
      rc=$?
      failed+=("$name")
      warn "$name: $command failed (exit $rc)"
    fi
  done

  (( ran > 0 )) || die "No instances exist. Run: ./hermes-spawn.sh spawn"

  printf '\n'
  if (( ${#failed[@]} > 0 )); then
    warn "Failed on ${#failed[@]}/$ran: $(printf '%s ' "${failed[@]}")"
    return 1
  fi
  info "Completed '$command' on all $ran instance(s)."
  return 0
}

main() {
  local command="${1:-spawn}" requested="${2:-}" dir
  case "$command" in
    -h|--help|help) usage; return ;;
    spawn|create) spawn_instance; return ;;
    list|ls) host_preflight; list_instances; return ;;
    host-preflight) host_preflight; return ;;
    preflight)
      exec python3 -B "$SCRIPT_DIR/lib/fleet_preflight.py" --instance-dir "$INSTANCES_DIR/$requested" --repo "$SCRIPT_DIR" "${@:3}"
      ;;
    fleet-doctor|formation-status)
      # Pure on-disk read-only checks. No host preflight, instance load, lock,
      # compose, or directory resolution that could mutate the instance.
      [[ -n "$requested" ]] || die "Usage: $command instance|all (instance is required)"
      local -a audit_args=()
      if [[ "$requested" == all ]]; then
        audit_args=(--instances-dir "$INSTANCES_DIR")
      else
        validate_hostname "$requested" || die "Unsafe instance name: $requested"
        audit_args=(--instance-dir "$INSTANCES_DIR/$requested")
      fi
      if [[ "$command" == formation-status ]]; then
        [[ $# == 2 ]] || die "Usage: formation-status instance|all"
        exec python3 -B "$SCRIPT_DIR/lib/formation_status.py" "${audit_args[@]}"
      fi
      exec python3 -B "$SCRIPT_DIR/lib/fleet_doctor.py" "${audit_args[@]}" "${@:3}"
      ;;
    supervisor) supervisor_command "${@:2}"; return ;;
    start|stop|restart|status|logs|setup|config|chat|shell|doctor|serve|reauth|credentials|update|apply-stack|stack-status|audit-isolation|info|webui-ensure|webui-status|install-skills|add-persona|remove|delete) ;;
    *) usage >&2; die "Unknown command: $command" ;;
  esac

  # 'all' (or -a/--all) in the instance slot fans the command out to every
  # instance instead of naming one. Handled before preflight/lock so each
  # per-instance run does its own preflight and takes its own lock.
  case "$requested" in
    all|-a|--all) run_all_command "$command" "${@:3}"; return $? ;;
  esac

  # Audits must not turn prerequisite failures into drift or write lock files.
  if [[ "$command" == stack-status ]]; then ERROR_EXIT_CODE=2; else host_preflight; fi
  dir="$(resolve_instance "$requested")"
  readonly INSTANCE_DIR="$dir"
  export INSTANCE_DIR
  load_instance "$dir"

  if [[ "$command" == stack-status ]]; then
    stack_status_command "${@:3}"
    return $?
  fi

  # Prevent two lifecycle operations from racing on one instance. Time out
  # loudly with the holder named, instead of waiting forever.
  exec 9>"$INSTANCE_DIR/.operation.lock"
  if ! flock -w 20 9 2>/dev/null; then
    holders="$(fuser "$INSTANCE_DIR/.operation.lock" 2>/dev/null | xargs || true)"
    die "Instance $TAILSCALE_HOSTNAME is busy (lock held by PID(s): ${holders:-unknown}). Close that operation and retry."
  fi

  case "$command" in
    start) ensure_running; lock_bot_relay; configure_serve || true ;;
    stop) compose down ;;
    restart) compose restart; wait_for_tailscale; lock_bot_relay ;;
    status)
      compose ps
      compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock status || true
      compose exec -T tailscale tailscale --socket=/tmp/tailscaled.sock serve status || true
      ;;
    logs) compose logs --follow --tail=150 ;;
    setup|config) setup_command ;;
    chat) ensure_running; compose exec hermes hermes -p "$HERMES_PROFILE" ;;
    shell) ensure_running; compose exec hermes bash ;;
    doctor) ensure_running; compose exec -T hermes hermes -p "$HERMES_PROFILE" doctor ;;
    serve) ensure_running; configure_serve ;;
    reauth) reauth_command ;;
    credentials) ensure_running; credentials_command ;;
    apply-stack) apply_stack_command "${@:3}" ;;
    install-skills) install_skills_command ;;
    add-persona) add_persona_command "${@:3}" ;;
    remove|delete) remove_command "${@:3}" ;;
    webui-ensure) webui_ensure_command ;;
    webui-status) webui_status_command ;;
    stack-status) stack_status_command; exit $? ;;
    audit-isolation) audit_isolation_command; exit $? ;;
    update)
      info "Pulling current images and recreating $TAILSCALE_HOSTNAME..."
      compose pull
      compose up -d --remove-orphans
      wait_for_tailscale
      configure_serve || true
      ;;
    info) sed -n '1,200p' "$INSTANCE_DIR/system-info.txt" ;;
  esac
}

main "$@"
